...
public class ParameterToolbox {
  ...
  private static List<Map<String, Object>> getParameters(boolean inputParameters) {
    ...
    RunnerParameter chooseFunction = new RunnerParameter(KeycloakInput.INPUT_KEYCLOAK_FUNCTION, "Keycloak Function",
    ...
    if (inputParameters) {
      ...
      for (RunnerParameter runnerParameter : KeycloakFunction.getInputCommonParameters()) {
    ...
    return parametersCollectList.stream().map(t -> t.toMap(KeycloakInput.INPUT_KEYCLOAK_FUNCTION)).toList();
